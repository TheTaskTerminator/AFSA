"""Backend Agent implementation for API and database code generation."""

import json
import logging
import uuid
from dataclasses import dataclass, field
from datetime import datetime, timezone
from typing import Any, Dict, List, Optional

from app.agents.base import AgentResponse, AgentType, BaseAgent, TaskCard
from app.agents.llm import BaseLLM, ChatMessage, get_llm
from app.agents.backend_agent.prompts import (
    API_GENERATION_PROMPT,
    BACKEND_SYSTEM_PROMPT,
    detect_api_type,
    extract_model_name,
    get_system_prompt,
)
from app.agents.backend_agent.tools import (
    APIGenerationResult,
    APIGenerationTool,
    CodeReviewTool,
    GeneratedFile,
    SchemaGenerationTool,
)

logger = logging.getLogger(__name__)


@dataclass
class BackendSession:
    """Session for backend code generation."""

    session_id: str
    messages: List[Dict[str, str]] = field(default_factory=list)
    generated_files: List[GeneratedFile] = field(default_factory=list)
    context: Dict[str, Any] = field(default_factory=dict)
    is_complete: bool = False
    created_at: datetime = field(default_factory=lambda: datetime.now(timezone.utc))
    updated_at: datetime = field(default_factory=lambda: datetime.now(timezone.utc))


class BackendAgent(BaseAgent):
    """Backend Development Agent for API and database code generation.

    The Backend Agent is responsible for:
    1. Understanding API requirements
    2. Generating FastAPI endpoints
    3. Creating SQLAlchemy models
    4. Implementing business logic

    Attributes:
        agent_type: Always AgentType.BACKEND
        name: Agent name for identification
        llm: LLM instance for code generation
        tools: Tool instances for code operations
    """

    agent_type = AgentType.BACKEND
    name = "Backend Agent"

    def __init__(
        self,
        llm: Optional[BaseLLM] = None,
        config: Optional[Dict[str, Any]] = None,
    ):
        """Initialize Backend Agent.

        Args:
            llm: LLM instance (will use default if not provided)
            config: Agent configuration
        """
        self._llm = llm
        self._config = config or {}
        self._sessions: Dict[str, BackendSession] = {}

        # Tools will be initialized when LLM is available
        self._api_tool: Optional[APIGenerationTool] = None
        self._schema_tool: Optional[SchemaGenerationTool] = None
        self._review_tool: Optional[CodeReviewTool] = None

    @property
    def llm(self) -> BaseLLM:
        """Get LLM instance."""
        if self._llm is None:
            self._llm = get_llm()
        return self._llm

    def _ensure_tools(self) -> None:
        """Ensure tools are initialized."""
        if self._api_tool is None:
            self._api_tool = APIGenerationTool(self.llm)
        if self._schema_tool is None:
            self._schema_tool = SchemaGenerationTool(self.llm)
        if self._review_tool is None:
            self._review_tool = CodeReviewTool(self.llm)

    def _get_or_create_session(self, session_id: str) -> BackendSession:
        """Get existing session or create new one."""
        if session_id not in self._sessions:
            self._sessions[session_id] = BackendSession(session_id=session_id)
        return self._sessions[session_id]

    async def process_message(
        self,
        session_id: str,
        message: str,
        context: Optional[Dict[str, Any]] = None,
    ) -> AgentResponse:
        """Process a user message for code generation.

        Args:
            session_id: Session identifier
            message: User message describing API requirements
            context: Additional context (existing models, schemas, etc.)

        Returns:
            AgentResponse with generated code or clarification questions
        """
        self._ensure_tools()
        session = self._get_or_create_session(session_id)

        # Update context if provided
        if context:
            session.context.update(context)

        # Add user message to history
        session.messages.append({
            "role": "user",
            "content": message,
            "timestamp": datetime.now(timezone.utc).isoformat(),
        })

        try:
            # Detect API type and model name
            api_type = detect_api_type(message)
            model_name = extract_model_name(message)

            # Build messages for LLM
            llm_messages = self._build_llm_messages(session, message)

            # Get LLM response
            response = await self.llm.chat(llm_messages, temperature=0.3)
            content = response.content

            # Parse response
            parsed = self._parse_response(content)

            # Add assistant message to history
            session.messages.append({
                "role": "assistant",
                "content": content,
                "timestamp": datetime.now(timezone.utc).isoformat(),
            })
            session.updated_at = datetime.now(timezone.utc)

            if parsed.get("type") == "clarification":
                return AgentResponse(
                    success=True,
                    content="我需要澄清一些细节：",
                    clarification_questions=parsed.get("questions", []),
                    metadata={"session_id": session_id, "type": "clarification"},
                )

            elif parsed.get("type") == "code_generation":
                files_data = parsed.get("files", [])
                generated_files = [
                    GeneratedFile(
                        path=f.get("path", f"app/api/v1/{model_name.lower()}.py"),
                        content=f.get("content", ""),
                        description=f.get("description", ""),
                        language=f.get("language", "python"),
                    )
                    for f in files_data
                ]

                session.generated_files.extend(generated_files)

                # Review generated code
                review_results = []
                for file in generated_files:
                    if file.language == "python":
                        review = await self._review_tool.review_code(
                            file.content,
                            self._detect_file_type(file.path),
                        )
                        review_results.append({
                            "file": file.path,
                            "review": review,
                        })

                return AgentResponse(
                    success=True,
                    content=f"已生成 {len(generated_files)} 个文件",
                    metadata={
                        "session_id": session_id,
                        "type": "code_generation",
                        "files": [{"path": f.path, "description": f.description} for f in generated_files],
                        "dependencies": parsed.get("dependencies", []),
                        "migrations": parsed.get("migrations", []),
                        "review": review_results,
                    },
                )

            else:
                # Regular response
                return AgentResponse(
                    success=True,
                    content=content,
                    metadata={"session_id": session_id},
                )

        except Exception as e:
            logger.error(f"Backend Agent error: {e}")
            return AgentResponse(
                success=False,
                content=f"代码生成时出错：{str(e)}",
                metadata={"error": str(e), "session_id": session_id},
            )

    async def generate_task_card(self, session_id: str) -> Optional[TaskCard]:
        """Generate a structured task card from the conversation.

        Args:
            session_id: Session identifier

        Returns:
            TaskCard if successfully generated, None otherwise
        """
        session = self._sessions.get(session_id)

        if not session or not session.messages:
            return None

        # Create task card from the generation session
        task_card = TaskCard(
            id=str(uuid.uuid4()),
            type="feature",
            priority="medium",
            description="API 代码生成任务",
            requirements=[],
            constraints={
                "technology_stack": ["FastAPI", "SQLAlchemy", "Pydantic", "PostgreSQL"],
            },
        )

        return task_card

    async def execute(self, task_card: TaskCard) -> AgentResponse:
        """Execute code generation for the task.

        Args:
            task_card: Task card with API requirements

        Returns:
            AgentResponse with generated code
        """
        self._ensure_tools()

        try:
            # Extract requirements from task card
            description = task_card.description
            requirements = task_card.requirements
            constraints = task_card.constraints

            # Detect what to generate
            api_type = detect_api_type(description)
            model_name = extract_model_name(description)

            # Generate code based on type
            if api_type == "crud":
                # Generate full CRUD API
                fields = constraints.get("fields", [
                    {"name": "name", "type": "string"},
                ])
                result = await self._api_tool.generate_crud_api(
                    model_name=model_name,
                    fields=fields,
                    constraints=constraints,
                )
            else:
                # Generate specific endpoint
                result = await self._api_tool.generate_endpoint(
                    path=f"/api/v1/{model_name.lower()}s",
                    method="GET",
                    description=description,
                )

            if result.errors:
                return AgentResponse(
                    success=False,
                    content="代码生成失败",
                    metadata={"errors": result.errors},
                )

            # Review generated code
            review_results = []
            for file in result.files:
                if file.language == "python":
                    review = await self._review_tool.review_code(
                        file.content,
                        self._detect_file_type(file.path),
                    )
                    security = await self._review_tool.check_security(file.content)
                    review_results.append({
                        "file": file.path,
                        "review": review,
                        "security": security,
                    })

            return AgentResponse(
                success=True,
                content=f"成功生成 {len(result.files)} 个文件",
                metadata={
                    "files": [{"path": f.path, "content": f.content} for f in result.files],
                    "dependencies": result.dependencies,
                    "migrations": result.migrations,
                    "review": review_results,
                },
                task_card=task_card,
            )

        except Exception as e:
            logger.error(f"Backend Agent execution error: {e}")
            return AgentResponse(
                success=False,
                content=f"执行失败：{str(e)}",
                metadata={"error": str(e)},
            )

    def _build_llm_messages(
        self,
        session: BackendSession,
        current_message: str,
    ) -> List[ChatMessage]:
        """Build message list for LLM."""
        messages = [ChatMessage(role="system", content=get_system_prompt())]

        # Add existing code context if available
        if session.generated_files:
            existing_code = "\n\n".join(
                f"# {f.path}\n{f.content}"
                for f in session.generated_files[-3:]  # Last 3 files
            )
            messages.append(ChatMessage(
                role="system",
                content=f"已生成的代码：\n{existing_code}",
            ))

        # Add conversation history (limit to last 8 messages)
        for msg in session.messages[-8:]:
            messages.append(ChatMessage(
                role=msg["role"],
                content=msg["content"],
            ))

        return messages

    def _parse_response(self, content: str) -> Dict[str, Any]:
        """Parse LLM response for structured output."""
        import re

        # Try to find JSON in the response
        json_pattern = r"```json\s*([\s\S]*?)\s*```"
        matches = re.findall(json_pattern, content)

        if matches:
            try:
                return json.loads(matches[0])
            except json.JSONDecodeError:
                pass

        # Try direct JSON parse
        try:
            return json.loads(content)
        except json.JSONDecodeError:
            pass

        # No structured output found
        return {"type": "response", "content": content}

    def _detect_file_type(self, path: str) -> str:
        """Detect file type from path."""
        if "/api/" in path:
            return "api"
        elif "/models/" in path:
            return "model"
        elif "/schemas/" in path:
            return "schema"
        elif "/repositories/" in path:
            return "repository"
        return "python"

    def get_generated_files(self, session_id: str) -> List[GeneratedFile]:
        """Get all generated files for a session.

        Args:
            session_id: Session identifier

        Returns:
            List of generated files
        """
        session = self._sessions.get(session_id)
        if not session:
            return []
        return session.generated_files

    def clear_session(self, session_id: str) -> bool:
        """Clear a session from memory."""
        if session_id in self._sessions:
            del self._sessions[session_id]
            return True
        return False