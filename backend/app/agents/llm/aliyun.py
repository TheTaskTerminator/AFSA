"""Alibaba Cloud (阿里云百炼) LLM provider implementation.

阿里云百炼 uses OpenAI-compatible API with custom base URL.
"""
...
class AliyunLLM(BaseLLM):
    """Alibaba Cloud (阿里云百炼) LLM provider.

    阿里云百炼 provides OpenAI-compatible API endpoints.
    Model examples: qwen-max, qwen-plus, qwen-turbo, etc.
    """
    ...
    def _get_client(self) -> AsyncOpenAI:
        """Get or create Aliyun client."""
        if self._client is None:
            base_url = self.config.base_url or ALIYUN_DEFAULT_BASE_URL
    ...
    async def chat_stream(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            logger.error(f"Aliyun stream error: {e}")
            raise

    async def count_tokens(self, text: str) -> int:
        """Count tokens (approximation for Chinese text)."""
        # Qwen models are optimized for Chinese
